...
class ProjectModuleManager {
    static final String PLUGIN_NAME = "BusGradlePlugin"
    ...
    static boolean manageModule(Project project) {
        ...
        println "${PLUGIN_NAME}: mainModuleName=${mainModuleName}, project=${project.name}, runAsApp=${runAsApp} . taskIsAssemble:${taskIsAssemble}. " +
                "settings(mainApp:${mainApp}, alwaysLib:${alwaysLib}, assembleThisModule:${assembleFor})"
        if (runAsApp) {
...
//        project.logger.error("tasks:"+taskNames)
        ...
        project.ext.addComponent = { dependencyName, realDependency = null ->
            ...
            if (!excludeModule) {
                ...
                if (realDependency) {
                    ...
                    println PLUGIN_NAME + " >>>> add $realDependency to ${project.name}'s dependencies"
                } else if (componentProject) {
                    //第二个参数未传，默认为按照module来进行依赖
                    project.dependencies.add(dependencyMode, project.project(":$dependencyName"))
                    println PLUGIN_NAME + " >>>> add project(\":$dependencyName\") to ${project.name}'s dependencies"
                } else {
                    throw new RuntimeException(
                            PLUGIN_NAME + " >>>> add dependency by [ addComponent '$dependencyName' ] occurred an error:" +
